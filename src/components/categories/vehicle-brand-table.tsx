...
import { useVehicleBrands, useDeleteVehicleBrand } from '@/hooks/use-supabase';
import { Badge } from '@/components/ui/badge';
import { Search, Plus, ArrowUpDown, Pencil, Trash2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { VehicleBrandFormDialog } from './vehicle-brand-form-dialog';
import { toast } from 'sonner';

const columnHelper = createColumnHelper<VehicleBrand>();

export function VehicleBrandTable() {
  const { data: vehicleBrands = [], isLoading } = useVehicleBrands();
  const deleteVehicleBrand = useDeleteVehicleBrand();
  ...
  const filteredVehicleBrands = useMemo(() => {
    return searchQuery
      ...
      : vehicleBrands;
  }, [vehicleBrands, searchQuery, fuse]);

  const handleDelete = async (e: React.MouseEvent, brand: VehicleBrand) => {
    e.stopPropagation();
    if (confirm(`¿Estás seguro de que quieres eliminar la marca "${brand.name}"?`)) {
      try {
        await deleteVehicleBrand.mutateAsync(brand.id);
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg border border-slate-200 shadow-sm max-w-4xl mx-auto w-full">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-200">
        <div className="flex items-center gap-3">
          <h2 className="text-[18px] font-bold text-slate-900">Gestor de Marcas de Vehículos</h2>
          <Badge variant="secondary" className="bg-slate-100 text-slate-700 text-[10px] font-bold uppercase tracking-widest border-none px-3 py-1 rounded-md">
            {filteredVehicleBrands.length} MARCAS DE VEHÍCULOS
          </Badge>
        </div>
